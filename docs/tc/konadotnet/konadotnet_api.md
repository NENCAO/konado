...
> 此功能仍屬於實驗性功能，可能存在一些問題。

## 簡介

Konado.NET 是 Konado 對話系統的 C# API 擴充。透過 Konado.NET，開發者可以在 C# 專案中輕鬆建立、管理和執行對話內容。

## 使用方法

### 非 .NET 專案

如果在非 .NET 專案中使用 Konado .NET API，會遇到如下報錯，這是正常現象：
```
...
此問題不會影響 Konado 基礎功能的使用，但無法使用 Konado .NET API。
...
```
無法從路徑 “res://addons/konadotnet/Konadotnet.cs” 載入附加元件腳本：該腳本可能有程式碼錯誤。
正在禁用位於 “res://addons/konadotnet/plugin.cfg” 的附加元件以阻止其進一步報錯。
```
...
這是正常現象，請重新在 Godot 編譯 Konado.NET，然後重新開啟專案即可解決。

如果無法啟用插件，並且 MSBuild 中沒有任何報錯，可以嘗試關閉專案後，刪除專案根目錄的 `.godot/` 資料夾，然後重新生成專案。
...
Wrapper 類別提供了對 GDScript 物件的 C# 封裝，使開發者可以在 C# 中操作 Konado 的各種資料結構。不過目前這些類別尚未完全實作，僅提供部分屬性與方法，有待進一步完善。
...
- `string Name`: 場景名稱
- `string ShotId`: 場景 ID
- `string SourceStory`: 來源故事
- `Array<Dictionary> DialoguesSourceData`: 對話來源資料
- `Dictionary Branches`: 分支
- `Dictionary<string, Dictionary> SourceBranches`: 來源分支
- `Dictionary<string, int> ActorCharacterMap`: 角色映射

#### KonadoScriptsInterpreter

KonadoScriptsInterpreter 腳本解譯器包裝器，用於解析 Konado 腳本檔案。

##### 方法

- `KndShot ProcessScriptsToData(string path)`: 將腳本檔案處理為資料
